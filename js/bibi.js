...
function loadData(name, time) {
    ...
    if (d) {
        ...
        if (-1 < t && t < (time * 6000)) return d.data;
    }
    return false;
};
...
function indexTalk() {
    ...
    function talk(ls) {
        ...
        document.querySelectorAll('#bber-talk .swiper-slide').forEach((el, index) => {
    ...
    let d = loadData('talk', 5);
    if (d) talk(d);
    else {
        fetch('https://memos.mgodmonkey.love/api/memo?creatorId=1&tag=通告&limit=10').then(res => res.json()).then(data => { // 更改地址和ID
