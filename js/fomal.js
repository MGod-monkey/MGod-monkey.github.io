...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | MGodmonkeyの世界")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'OASBZ-HJ43N-SM2FU-SRTTL-PYL6F-G4B7H',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(110.416742, 25.311923, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://s2.loli.net/2023/03/20/l5Gj2m8yZXvDISP.png",
        hoverMsg: "抓到你啦~",
...
function createtime1() {
  var grt = new Date("17/03/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
`,
    "小站已经苟活",
    dnum,
    "©2023 By MGodmonkey, crtl+cv form Fomalhaut",
  ];
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by MGodmonkey %c 你正在访问MGodmonkeyの世界",
...
function switchNightMode() {
  document.querySelector('body').insertAdjacentHTML('beforeend', '<div class="Cuteen_DarkSky"><div class="Cuteen_DarkPlanet"><div id="sun"></div><div id="moon"></div></div></div>'),
    setTimeout(function () {
      document.querySelector('body').classList.contains('DarkMode') ? (document.querySelector('body').classList.remove('DarkMode'), localStorage.setItem('isDark', '0'), document.getElementById('modeicon').setAttribute('xlink:href', '#icon-yintian')) : (document.querySelector('body').classList.add('DarkMode'), localStorage.setItem('isDark', '1'), document.getElementById('modeicon').setAttribute('xlink:href', '#icon-qingtian')),
        setTimeout(function () {
  ...
  if (nowMode === 'light') {
    ...
    document.getElementById('modeicon').setAttribute('xlink:href', '#icon-qingtian')
    // 延时弹窗提醒
    setTimeout(() => {
      new Vue({
        data: function () {
    ...
    document.querySelector('body').classList.add('DarkMode'), document.getElementById('modeicon').setAttribute('xlink:href', '#icon-yintian')
    setTimeout(() => {
      new Vue({
        data: function () {
          this.$notify({
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| MGodmonkey") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('MGodmonkey的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
map.set('lightgreen', "rgb(111, 170, 32)");
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("03/17/2023 00:00:00");
  ...
  let currentTimeHtml = "";
  (currentTimeHtml =
    `本破站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i>`),
  ...
  setTimeout(function () {
    new Vue({
      data: function () {
        this.$notify({
          title: "温馨提示",
          message: "由于网站部分设置项更新，当前已为您重置所有设置，祝您愉快！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "Smiley");
}
...
function setFont(n) {
  ...
  if (n == "default") {
    ...
    document.body.style.fontFamily = "-apple-system, Consolas_1, BlinkMacSystemFont, DingTalk, Source_Han, Smiley, 'Segoe UI', 'PingFang SC' , 'Microsoft JhengHei' , 'Microsoft YaHei' , sans-serif";
  }
  else {
    document.documentElement.style.setProperty('--global-font', n);
    document.body.style.fontFamily = "var(--global-font),-apple-system, DingTalk, Source_Han, Smiley, monosapce,'微软雅黑', sans-serif";
  }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://mybog.s3.bitiful.net/imgs/JaEtOo8VKfpsh5M.jpg);
    --darkmode-bg: url(https://mybog.s3.bitiful.net/imgs/JaEtOo8VKfpsh5M.jpg);
    --mobileday-bg: url(https://mybog.s3.bitiful.net/imgs/JaEtOo8VKfpsh5M.jpg);
    --mobilenight-bg: url(https://mybog.s3.bitiful.net/imgs/JaEtOo8VKfpsh5M.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://api.zzzmh.cn/bz/v3/getUrl/97b97ee1173042d5ba667df7ecbe43f610"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
